...
Gebruik ALTIJD de volgende naamgevingsconventie voor je scripts:
...
Sorteer bestanden in Windows Explorer op "Date modified" om de meest recente versies bovenaan te zien.
...
Je hoeft packages maar **één keer** te installeren per computer, maar je moet ze **elke keer** laden wanneer je ze wilt gebruiken.
...
# R Symbolen en Operators (Essentiële Basis)

Voordat je begint met programmeren in R, moet je de belangrijkste symbolen begrijpen. Deze vormen de bouwstenen van elke R code.

Dit zijn alleen de **4 fundamentele symbolen** voor beginners. R heeft veel meer operators en functies die je in latere hoofdstukken leert tijdens data-analyse, statistiek, en visualisatie.

## De 4 Basis Symbolen

### 1. `<-` - Assignment Operator (Toekenning)

**Wat doet het?** Bewaar waarden in variabelen ("ken toe aan" / "opslaan in")
...
# Voorbeelden:
leeftijd <- 25                          # Bewaar getal 25
naam <- "Jan"                           # Bewaar tekst "Jan"
```

### 2. `$` - Kolom Selector

**Wat doet het?** Haal een specifieke kolom uit een dataset
...
# Voorbeelden:
studenten_data$leeftijd        # Haal leeftijd kolom
crime_data$misdrijf_type       # Haal misdrijf type kolom

# Combinatie met <-
data$nieuwe_kolom <- data$oude_kolom * 2  # Maak nieuwe kolom
```

### 3. `|>` - Native Pipe Operator (Modern, R 4.1+)

**Wat doet het?** Stuur resultaat door van links naar rechts ("en dan...")
...
# Is hetzelfde als:
functie2(functie1(data))

# Praktisch voorbeeld:
studenten_data |> 
  filter(leeftijd > 18) |>           # Stap 1: Filter volwassenen
  select(naam, cijfer) |>            # Stap 2: Selecteer kolommen
  arrange(cijfer)                    # Stap 3: Sorteer op cijfer
```

### 4. `%>%` - Magrittr Pipe (Ouder, dplyr/tidyverse)

**Wat doet het?** Precies hetzelfde als `|>` maar voor oudere R versies

```r
# Gebruik alleen als je R versie < 4.1 hebt
studenten_data %>% 
  filter(leeftijd > 18) %>%          # Stap 1: Filter
  select(naam, cijfer)               # Stap 2: Selecteer
```

## Belangrijke R Operators - Overzicht

### 1. Assignment Operators (Toewijzing)

| Operator | Gebruik | Voorbeeld |
|----------|---------|-----------|
| `<-` | Waarde toewijzen aan variabele (standaard) | `x <- 10` |
| `=` | Waarde toewijzen (vooral in functies) | `mean(x = c(1,2,3))` |
| `->` | Rechts toewijzen (zeldzaam) | `10 -> x` |
...
### 2. Access Operators (Toegang tot data)

| Operator | Gebruik | Voorbeeld |
|----------|---------|-----------|
| `$` | Kolom uit dataframe halen | `data$leeftijd` |
| `[]` | Selectie via index | `data[1, 2]` (rij 1, kolom 2) |
| `[[]]` | Enkel element uit lijst | `lijst[[1]]` |

```r
# Voorbeelden
leeftijden <- personen$leeftijd          # Hele kolom
eerste_persoon <- personen[1, ]          # Eerste rij
naam_kolom <- personen[, "naam"]         # Kolom via naam
specifiek <- personen[5, 3]              # Rij 5, kolom 3
```

### 3. Pipe Operators (Workflow)

| Operator | Gebruik | Voorbeeld |
|----------|---------|-----------|
| `|>` | Native pipe (R 4.1+) - **aanbevolen** | `data |> filter() |> select()` |
| `%>%` | Magrittr pipe (tidyverse) | `data %>% filter() %>% select()` |

```r
# Voorbeeld zonder pipe (moeilijk leesbaar)
resultaat <- select(filter(data, leeftijd > 18), naam, leeftijd)

# Voorbeeld met pipe (duidelijk)
resultaat <- data |>
  filter(leeftijd > 18) |>
  select(naam, leeftijd)
```

### 4. Comparison Operators (Vergelijking)
...
| `>` | Groter dan | `x > 10` |
| `<` | Kleiner dan | `x < 10` |
| `>=` | Groter of gelijk | `x >= 10` |
| `<=` | Kleiner of gelijk | `x <= 10` |
| `%in%` | In een set | `x %in% c(1,2,3)` |

```r
# Voorbeelden
data |> filter(leeftijd >= 18)           # Volwassenen
data |> filter(stad == "Gent")           # Alleen Gent
data |> filter(score != 0)               # Geen nullen
data |> filter(land %in% c("BE", "NL"))  # België of Nederland
```

### 5. Logical Operators (Logische combinaties)

| Operator | Gebruik | Voorbeeld |
|----------|---------|-----------|
| `&` | EN (vectorized) | `x > 5 & x < 10` |
| `|` | OF (vectorized) | `x < 5 | x > 10` |
| `!` | NIET | `!(x == 5)` |
| `&&` | EN (single value) | Alleen in if-statements |
| `||` | OF (single value) | Alleen in if-statements |

```r
# Voorbeelden
data |> filter(leeftijd > 18 & stad == "Gent")     # EN conditie
data |> filter(leeftijd < 18 | leeftijd > 65)     # OF conditie
data |> filter(!is.na(score))                      # Geen missing values
```

### 6. Arithmetic Operators (Rekenen)
...
data |> mutate(
  ...
  percentage = (waarde / totaal) * 100
)
```

### 7. Special Operators (Speciaal)

| Operator | Gebruik | Voorbeeld |
|----------|---------|-----------|
| `:` | Sequence maken | `1:10` (getallen 1 tot 10) |
| `::` | Pakket functie gebruiken | `dplyr::filter()` |
| `~` | Formula | `y ~ x` (in modellen) |

```r
# Voorbeelden
jaren <- 2020:2024                      # Sequence
dplyr::select(data, naam)               # Expliciete pakket functie
model <- lm(gewicht ~ lengte, data)     # Linear model
```

## Praktische Toepassingen

### Voorbeeld 1: Data Inladen en Verkennen
```r
# Assignment en access operators
studenten <- read.csv("studenten.csv")
leeftijden <- studenten$leeftijd
eerste_rij <- studenten[1, ]
```

### Voorbeeld 2: Data Filteren
```r
# Pipe + comparison + logical operators
volwassen_studenten <- studenten |>
  filter(leeftijd >= 18 & stad %in% c("Gent", "Brussel")) |>
  select(naam, leeftijd, stad)
```

### Voorbeeld 3: Nieuwe Kolommen Maken
```r
# Pipe + arithmetic operators
studenten_verrijkt <- studenten |>
  mutate(
    leeftijd_maanden = leeftijd * 12,
    categorie = ifelse(leeftijd >= 18, "Volwassen", "Minderjarig"),
    groep = leeftijd %/% 10  # Leeftijdsgroep (0-9, 10-19, etc.)
  )
```

### Voorbeeld 4: Combinatie van Alles
```r
# Complete workflow
analyse <- data |>
  filter(jaar >= 2020 & !is.na(score)) |>
  mutate(
  ...
  summarise(
    aantal = n(),
    gem_score = mean(score)
  )

# Extra kolom toevoegen
analyse$datum <- Sys.Date()
```

## Praktijkvoorbeeld: Complete Workflow

```r
# Stap 1: Laden en opslaan (← operator)
mijn_data <- read.csv("data/onderzoek.csv")

# Stap 2: Kolom bekijken ($ operator)
mijn_data$leeftijd

# Stap 3: Data bewerken (|> pipe)
schone_data <- mijn_data |>
  filter(leeftijd > 18) |>
  select(id, leeftijd)

# Stap 4: Nieuwe kolom ($ en <-)
schone_data$volwassen <- "Ja"
```

## Meer Symbolen in Latere Hoofdstukken

Je leert later ook:
- **Vergelijken:** `==`, `!=`, `>`, `<` (gelijk aan, niet gelijk aan)
- **Logica:** `&` (en), `|` (of)
- **Checken:** `%in%` (zit waarde in lijst?)
- **Subsetten:** `[]` (selecteer specifieke rijen/kolommen)
- **Packages:** `::` (gebruik functie uit package)
- **Modellen:** `~` (formules voor statistiek)

**Focus nu op de 4 basis symbolen** - de rest komt wanneer je ze nodig hebt!

# Eerste Stappen in R

Nu je weet hoe packages werken en de belangrijkste R symbolen begrijpt, kun je beginnen met het leren van de basis van R programmeren.

## Commentaar in R

Alles wat je typt na een `#` (hekje) wordt beschouwd als commentaar en wordt NIET uitgevoerd door R. 
...
Gebruik `# sectienaam ----` (minimaal 3 streepjes) om secties te maken in je script. Dit maakt je code overzichtelijker:
...
Variabelnamen zijn hoofdlettergevoelig. `x` en `X` zijn verschillende variabelen!
...
Begin met het leren van 3-5 shortcuts die je het meest gebruikt, en voeg geleidelijk meer toe naarmate je comfortabeler wordt.
